...
| Заглавный экран | Трюм и верстак (TAB) |
|---|---|
...
| колесо мыши, `1`…`6` | выбрать слот |
| `Tab` / `I` | трюм и верстак: посмотреть, что есть, и собрать вещь |
| `F` / `G` | поесть / попить |
...
`TAB` открывает трюм и верстак. Сверху — что лежит в трюме, ниже — что из этого
можно собрать: доступные рецепты горят тёплым, недоступные бледнеют. Наведи
курсор на рецепт — под сеткой появится строка «из чего он и чего не хватает»;
щёлкни — соберётся один предмет. Мир при этом продолжает жить: верстак не ставит
игру на паузу.
