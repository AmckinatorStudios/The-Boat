...
local function refreshCell(x, y, z)
    ...
    if Blocks.IsLight(id) then
        -- Сам блок СВЕТИТСЯ, а не просто стоит рядом с источником света.
        -- Точечный свет ниже освещает палубу вокруг, но без свечения самого
        -- куба фонарь ночью выглядит тёмным пятном в центре освещённого круга —
        -- источник света, который сам не светится.
        obj.Emissive = Vec3(1.0, 0.72, 0.34)
        -- Порог bloom'а — единица, так что ореол начинается сразу за ней, но
        -- РАСТЁТ он с силой: замеры яркости над фонарём дают 42 при 1.5, 70 при
        -- 2.2 и 93 при 3.0, оставаясь тёплой (R>G>B). 2.2 — там, где ореол уже
        -- ясно виден, а сердцевина ещё не выбелена тон-маппингом досуха.
        obj.EmissiveStrength = 2.2
