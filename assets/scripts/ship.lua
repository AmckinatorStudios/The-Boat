...
local function refreshCell(x, y, z)
    ...
    if Blocks.IsLight(id) then
        ...
        -- Чуть больше единицы, а не «побольше, чтоб ярче»: порог bloom'а — 1,
        -- и ореол появляется уже здесь, а вот на 2.6 все три канала уходили за
        -- предел, тон-маппинг сводил их в один белый, и тёплый фонарь светился
        -- ХОЛОДНЫМ белым пятном — ровно тем, чего от лампы не ждёшь.
        obj.EmissiveStrength = 1.5
