...
function S.UpdateSky()
    ...
    local low = math.max(0.0, 1.0 - math.abs(sy) * 3.0)      -- солнце у горизонта
    sage.volumetric.Set{
        enabled = wantVolumetrics,
        shafts = day > 0.05,
        clouds = true,
        -- Плотность дымки поднимаем к закату: именно она делает лучи видимыми.
        density = 0.007 + low * 0.026,
        intensity = 0.7 + low * 0.8,
        coverage = 0.50,
    }
end
